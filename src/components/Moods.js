...
export default function Moods() {
  ...
  const makeHappy = () => {
    /* STEP 4 */
    return setCurrentMood(happyMood);
  };
  const makeSad = () => {
    /* STEP 5 */
    return setCurrentMood(sadMood);
  };
  const reset = () => {
    /* STEP 6 */
    return setCurrentMood(initialMood);
  };

  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: currentMood == happyMood ? 'royalblue' : 'crimson' // STEP 2
  };

  return (
    <div className='widget-moods container'>
      <h2>Moods</h2>
      <div id='mood' style={style}>{currentMood}</div> {/* STEP 3 */}
      <div>
